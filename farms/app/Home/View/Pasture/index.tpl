...
<div class="kj" id="kj">
	<div class="box">
		<div class="box_two" id="box_two">
			...
			<!--返回农场-->
			<a href="{:U('Index/index')}" rel="external"><div class="return_farm" id="return_farm"></div></a>
			...
			<div class="ji_pop" id="ji_pop_box">
				...
				<div class="ji_box">					
					...
					<a href="{:U('Index/index')}" rel="external"><div class="ji_return"></div></a>									
				</div>  				
			</div>
			<!--鸡舍没买弹窗结束-->
				...
				<div class="body" id="body">
					<for start="0" end="$num">
						 <?php
							...
							$index = rand(10,90);
						 ?>
						 <if condition="$direction[0] eq 'left'">        
							<div onclick="ChickenState({$i})" id="ji_{$i}" state=0 direction="{$direction[0]}" harvest_time={$res[$i]['harvest_time']} type={$res[$i]['chicken_id']} class="donghua chicken_{$res[$i]['id']}" style="z-index:{$index-9};top:{$index}%;left:<?php echo rand(10,85);?>%;">
							   <span id="StateTitle_{$i}"></span>
							   <img id="img_{$i}" src="/farms/Public/Home/images/ji/{$res[$i]['chicken_id']}-right-0.png">
							</div>
						 <else/>
							<div onclick="ChickenState({$i})" id="ji_{$i}" state=0 direction="{$direction[0]}" harvest_time={$res[$i]['harvest_time']} type={$res[$i]['chicken_id']} class="donghua chicken_{$res[$i]['id']}" style="z-index:{$index-9};top:{$index}%;left:<?php echo rand(10,85);?>%;">
							   <span id="StateTitle_{$i}"></span>
							   <img id="img_{$i}" src="/farms/Public/Home/images/ji/{$res[$i]['chicken_id']}-left-0.png">
							</div>
						 </if>
					</for>
				</div>
