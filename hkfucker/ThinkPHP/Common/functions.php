...
function check_verifys($code, $id = 1){
    $verify = new \Think\Verify();
    return $verify->ajax_check($code, $id);
}
...
function replaceweek($str){
    ...
    $NeedWeek= explode(",",$str);

    $NeedWeekLenght = count($NeedWeek);

    foreach ($NeedWeek as $key=>$val){
        $HaveWeekArray[trim($val)] = trim($val);
    }

    $NeedWeekArray =  array_replace_recursive($HaveWeekArray,$WeekArray);

    foreach ($NeedWeekArray as $key=>$val){
        if(! in_array($key,$HaveWeekArray)){
            unset($NeedWeekArray[$key]);
        }
    }

    $NeedWeekStr = implode(",",$NeedWeekArray);
...
function page_array($count,$page,$array,$order){      
	...
	$result['array']	 =   $pagedata ;
	
	$result['now_oage']	 =   $page ;
	...
	if($page <= $countpage-2){
		...
		$result['end']	 =   $countpage ;	
	}
	
	$result['countpage']	 =   $countpage ;
	
	$result['last_page'] = ($page > 1)  ?  $page - 1 : 1;
		
	$result['next_page'] = ($page < $countpage)  ?  $page + 1 : $countpage;
