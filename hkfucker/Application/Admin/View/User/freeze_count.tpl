...
<block name="content">
    ...
    <div class="col-md-9 col-md-offset-1 col-xs-8 col-xs-offset-1" >
        <span style="">该用户直推{$num_st}人，团队有{$num_team}人<small><a href="{:U('User/freeze_count',array('user'=>$user))}">团队直推</a></small></span>
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination" ">
                <li>
                    <a href="{:U('User/freeze_count',array('p'=>$p-1,'user'=>$user))}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
                </li>
				<for start="$start" end="$end">
                    <if condition="$p eq $i">
                        <li class="active">
                            <a href="{:U('User/freeze_count',array('p'=>$p-1,'user'=>$user))}">{$i}</a>
                        </li>
                        <else/>
                        <li>
                            <a href="{:U('User/freeze_count',array('p'=>$i,'user'=>$user))}">{$i}</a>
                        </li>
                    </if>
                </for>
                <li>
                    <a href="{:U('User/freeze_count',array('p'=>$p+1,'user'=>$user))}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a>
                </li>
            </ul>
        </nav>
        </if>
