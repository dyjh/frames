            ...
            <if condition="$res['0'] eq ''">
                ...
                <volist name="res" id="re">
                    <tr>
						...
						<if condition="$re.coin_feeze eq ''"><td style="color:red;">历史太久远</td><else/><td>{$re.coin_feeze}</td></if>
						<td>{$re.time|date="Y-m-d H:i:s",###}</td>
						<if condition="$re.type eq 0"><td style="color:red;">金币添加操作</td><else/><td>金币扣除操作</td></if>
                    </tr>
                </volist>
            </if>
            </tbody>
