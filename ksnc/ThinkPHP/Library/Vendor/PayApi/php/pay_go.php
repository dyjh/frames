 ...
 * 获取表单数据
 * */
//$order_id = (string) date("YmdHis"); //您的订单Id号，你必须自己保证订单号的唯一性，银泰不会限制该值的唯一性
$order_id .= "_".$table_prefix;
...
//卡类支付
     ...
     * 提交数据
     * */
    include_once("shunfoo/ClassShunfoopay.php");
    $shunfoo = new shunfoo();
    ...
    switch ($result) {
        ...
        case '-999':
            ...
            break;
        default:
            print('未知的返回值, 请联系银泰官方！');
            break;
    }
} else if ('card_muti' == $payType) {
    include_once("shunfoo/ClassShunfoo.muti.php");
    ...
    switch ($result) {
        ...
        case '-2':
            ...
            break;
        case '-3':
            print('<script language="javascript">alert("卡密为重复提交，银泰系统不进行消耗且不进入下行流程。"); history.go(-1);</script>');
            break;
        case '-4':
            print("卡密不符合银泰定义的卡号密码面值规则，银泰系统不进行消耗且不进入下行流程。");
            break;
        case '-999':
            print('<script language="javascript">alert("对不起，银泰付接口维护中，请选择其他的充值方式！"); history.go(-1);</script>');
            break;
        default:
            print('未知的返回值, 请联系银泰官方！');
            break;
