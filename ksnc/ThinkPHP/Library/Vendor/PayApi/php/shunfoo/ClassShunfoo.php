...
函数:
	send()
		...
		调用示例：
			...
			$result	= $shunfoo->send();
	recive()
		接收银泰消息，这里会进行身份的验证，如果您自己要做更多后续的处理，请在您自己的程序中进行
		调用示例：
...
class shunfoo{
	const shunfoo_card_url			= 'http://pay.initepay.com/Card/';
	const shunfoo_card_search_url	= 'http://pay.initepay.com/Card/';
	...
	*/
	public function send(){
		//检查是否正确
		$error 	= 0;
		$msg		= '您调用银泰支付接口的参数有误，错误信息如下：';
		if(empty($this->parter)){
			$error 	= 1;
			$msg 	.= '<li>parter不能为空: 商户id，由银泰联盟分配</li>';
		}
		if(empty($this->type)){
			$error 	= 1;
